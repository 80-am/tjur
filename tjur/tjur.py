...
logging.basicConfig(filename=tjur_dir + '/log/tjur-' + str(datetime.timestamp(datetime.utcnow())) +
    '.log', format='%(asctime)s.%(msecs)06d %(message)s', datefmt='%Y-%m-%d %H:%M:%S', level=logging.INFO)
        ...
        if (strategy.calculate_buy_signal() and signal == 0):
            ...
            while (signal == 1):
                ...
                if ((sell_signal and stop_loss > latest_price)
                    ...
                    print(datetime.utcnow(), 'Margin:', str(round(pl, 3)) + '%')
